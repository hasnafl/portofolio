            ...
            <div class="card-inner">
                <div>
                    <div class="card-title-group align-start gx-3 mb-3">
                        ...
                        <div class="card-tools">
                            <nuxt-link to="/alamatpelanggan/create" class="btn btn-primary btn-dim"><em class="icon ni ni-plus"></em><span>Add New Data</span></nuxt-link>
                        </div>
                    </div>
                ...
                <div>
                    <div class="card-title-group align-start gx-3 mb-3 mt-4">
                        <div class="card-title">
                            <h6 class="title">Latest Costumer Address Created</h6>
                        </div>
                        <div class="card-tools">
                            <div class="drodown">
                                <a
                        ...
                        <table class="table table-tranx mt-4 is-compact">
                            <thead>
                                <tr class="tb-tnx-head">
                                    ...
                                    <th>
                                        <span class="d-md-inline-block d-none">Label Alamat</span>
                                    </th>
                                    <th>
                                        <span class="d-md-inline-block d-none">Detail Alamat</span>
                                    </th>
                                    <th>
                            ...
                            <tbody>
                                <tr class="tb-tnx-item" v-for="(address, index) in data.data" :key="index">
                                    <td>
                                        <span class="d-md-inline-block d-none">{{ index + data.from }}</span>
                                        <div class="d-md-none d-inline-block">
                                            <strong class="text-primary">{{ address.id }}</strong>
                                            <br>
                                            <span>{{ address.id }} / {{ address.address_costumercode }}</span>
                                            <br>
                                            <Label :active="address.is_active" />
                                        </div>
                                    </td>
                                    <td>
                                        <span class="d-md-inline-block d-none">
                                            <strong class="text-primary">{{ address.address_customercode }}</strong> -
                                            {{ address.customer_name}}
                                        </span>
                                    </td>
                                    <td>
                                        <span class="d-md-inline-block d-none">{{ address.address_label }}</span>
                                    </td>
                                    <td>
                                        <span class="d-md-inline-block d-none">{{ address.address_detail }}</span>
                                    </td>
                                    <td>
                                        <span class="d-md-inline-block d-none">
                                            <Label :active="address.is_active" />
                                        ...
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right dropdown-menu-xs">
                                                <ul class="link-list-plain">
                                                    <li><nuxt-link :to="`/alamatpelanggan/detail/${address.id}`">View</nuxt-link></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </td>
...
export default {
    ...
    methods: {
        ...
        fetch (resetPage = false) {
            ...
            this.$axios.$post('/api/v1/alamatpelanggan/p', filter)
                .then((response) => {
        ...
        downloadTemplate() {
        this.$axios
            .$get("/api/v1/alamatpelanggan/template", { responseType: "blob" })
            .then((response) => {
            const blob = new Blob([response]);
            saveAs(blob, "customeraddress_template.xlsx");
            });
        },
        exportData() {
        this.$axios
            .$get("/api/v1/alamatpelanggan/export", { responseType: "blob" })
            .then((response) => {
            const blob = new Blob([response]);
            saveAs(blob, "customeraddress_export.xlsx");
            });
        },
        importData() {
        ...
        this.$axios
            .$post(`/api/v1/alamatpelanggan/import`, formData, headers)
            .then((data) => {
