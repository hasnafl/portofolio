...
use App\Services\Master\Pelanggan\DatapelangganService;
use App\Repositories\Master\Pelanggan\DatapelangganRepository;
...
{
    public function __construct(Request $request, DatapelangganService $service, DatapelangganRepository $repository)
    {
    ...
    {
        $data = $this->repository->listbranch($this->request->all());
    ...
    public function store()
    {
        ...
        // $exists = Customers::whereCode($this->request->code)->first();

        // if (isset($exists)) {
        //     throw new \Exception('Customers Code '.$this->request->code. ' already exists.');
        // }

        $data = $this->service->create($this->request->all());

        $alamatpelanggan = new Alamatpelanggan;
        $alamatpelanggan->address_customercode = $this->request->customer_code;
        $alamatpelanggan->address_detail = $this->request->customer_address;
        $alamatpelanggan->address_label = "Alamat Utama";
        $alamatpelanggan->address_ismainaddress = 1;
        $alamatpelanggan->save();

        return response()->json([
            'success' => true,
            'message' => 'Customers '.$data->id.' created.',
    ...
    public function update($code)
    {
        ...
        if ($code != $this->request->id) {
            throw new \Exception('Customers Code cannot be changed.');
        }
        ...
        $alamatpelanggan = Alamatpelanggan::where('address_customercode', '=', $this->request->customer_code)
        ->where('address_label', '=', 'Alamat Utama')
        ->update(array('address_detail' => $this->request->address_detail));

        return response()->json([
            'success' => true,
            'message' => 'Customers '.$code.' updated.',
            'data' => $this->repository->show($code)
        ]);
    }
    ...
    public function delete($code)
    {
        ...
        return response()->json([
            'success' => true,
            'message' => 'Customers  '.$code.' deleted.',
        ]);
    }
    ...
    public function changeActive($code)
    {
       ...
       return response()->json([
            'success' => true,
            'message' => 'Customers ' ."$code". " " . $message,
            'data' => $this->repository->show($code)
        ]);
    ...
    public function import()
    {
        ...
        $rows = Excel::toArray(new DatapelangganImport, $this->request->file('file'));
    ...
    {
        return Excel::download(new DatapelangganExport, 'customerclass_export.xlsx');
    }
